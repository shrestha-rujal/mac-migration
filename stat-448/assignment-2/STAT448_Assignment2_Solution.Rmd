...
ggplot(tib, aes(r, var)) + geom_col() +
  labs(title = "Top positive/negative correlations with V104 (Actual sales price)",
...
corrplot(cor(num_df[, top_vars]), method = "color", type = "upper",
...
tibble(Method = c("Backward", "Stepwise"),
...
ctr <- attr(X_tr, 'scaled:center'); scl <- attr(X_tr, 'scaled:scale')
...
cv_lasso <- cv.glmnet(X_tr, y_tr, alpha = 1, nfolds = 30,
...
par(mfrow = c(1,1))
plot(NULL, xlab = expression(log(lambda)), ylab = "CV MSE",
...
tibble(Model = c('lambda.min', 'lambda.1se'),
       alpha = best_alpha,
